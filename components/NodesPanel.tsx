// NodesPanel is the palette where users can add new nodes to their flow. It shows a list
// of available node types (currently just text nodes) that can be dragged or clicked to add.
...
// Define what types of nodes are available in the palette
const nodeTypes = [
  ...
  return (
    <Card className="w-56 h-full p-4 flex flex-col gap-4 bg-white border border-gray-200 shadow-none">
      ...
      <div className="flex flex-col gap-2">
        {/* Render each available node type as a button */}
        {nodeTypes.map((node) => (
          <Button
            key={node.type}
            variant="outline"
